...
class BurningPaperPainter extends CustomPainter {
  ...
  void _paintPathsOnCanvas(Size size, Path hole, Path outline, Canvas canvas) {
    ...
    Path holePath =
        Path.combine(PathOperation.difference, Path()..addRect(rect), hole);

    Path outlinePath = Path.combine(PathOperation.difference, hole, outline);
    ...
    canvas.drawPath(
        outlinePath, Paint()..color = Colors.black.withOpacity(0.5));
  }
